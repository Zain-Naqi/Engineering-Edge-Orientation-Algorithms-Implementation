...
Graph Graph::from_edge_list(const std::string& path) {
    ...
    for (auto& [u,v] : raw_edges) g.add_edge(u, v);

    cout << "Loaded graph: n=" << g.n() << "  m=" << g.m()
         << "  density=" << g.density() << "\n";
    return g;
}

Graph Graph::from_pairs(int num_vertices,
    ...
    for (auto [u,v] : canon) g.add_edge(u, v);
    return g;
}
...
OrientationState Graph::fresh_orientation() const {
    ...
    for (const Edge& e : edges_) s.outdeg[e.u]++;
    return s;
}
