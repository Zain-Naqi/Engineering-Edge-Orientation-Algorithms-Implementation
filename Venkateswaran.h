...
static vector<int> bfs_find_path(
    const Graph& g,
    const OrientationState& s,
    const vector<int>& sources,         // vertices in S
    const unordered_set<int>& T_set,    // set for O(1) membership test
    vector<int>& parent_edge,           // edge used to reach v
    vector<int> parent_vtx,             // predecessor of v
    long long& edges_visited            // accumulator
) {
    queue<int> q;
    ...
    for (int src : sources) {
        if (parent_edge[src] == -1) {
            parent_edge[src] = -2;      // -2 = "source, no parent edge"
            parent_vtx[src] = -2;
            q.push(src);
    ...
    // Reconstruct path edge IDs (source → sink) by walking parent pointers
    vector<int> path;
    if (sink_found != -1) {
        int cur = sink_found;
...
inline VenkatResult venkateswaran(const Graph& g, OrientationState& s) {
    ...
    while (true) {
        ...
        vector<int> path = bfs_find_path(g, s, S_vec, T, parent_edge, parent_vtx, result.edges_visited);
